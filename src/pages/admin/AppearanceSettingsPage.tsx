...
export function AppearanceSettingsPage() {
  ...
  useEffect(() => {
    ...
    colorKeys.forEach((k) => {
      (extracted as unknown as Record<string, unknown>)[k as string] = (settings as unknown as Record<string, unknown>)[k as string];
    });
    setColors(extracted);
  ...
  const handleSave = async () => {
    setSaving(true);
    const { data, error } = await updateSettings(colors);
    ...
    if (!data) {
      toast.error('Aucune donnée retournée. Vérifiez les droits admin.');
      setSaving(false);
      return;
    }
    ...
    await logActivityWithDiff(
      { action: 'theme_change', entity_type: 'setting', entity_id: data.id },
      settings,
      after
    );
    ...
    return (
      <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors">
        <div className="relative shrink-0">
          <div
            className="w-10 h-10 rounded-xl border-2 border-white shadow-sm overflow-hidden"
        ...
        <div className="xl:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            <div className="flex border-b border-slate-200">
              <button
                ...
                className={`flex-1 py-3 text-sm font-medium transition-colors ${
                  activeTab === 'light' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-3 text-sm font-medium transition-colors ${
                  activeTab === 'dark' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
            ...
            <div className="p-4 space-y-2">
              {activeTab === 'light'
                ? LIGHT_COLORS.map((c) => (
                    <ColorPicker key={c.key as string} colorKey={c.key} label={c.label} description={c.description} />
                  ))
                : DARK_COLORS.map((c) => (
                    <ColorPicker key={c.key as string} colorKey={c.key} label={c.label} />
                  ))}
            </div>
          </div>
        </div>
        ...
        <div className="xl:col-span-1">
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden sticky top-6">
            ...
            <div className="p-4 transition-all duration-300" style={previewStyle as React.CSSProperties}>
              ...
              >
                <p className="text-xs font-bold mb-1" style={{ color: 'var(--preview-text)' }}>
                  Produits à la une
                </p>
                <p className="text-[10px] mb-2" style={{ color: 'var(--preview-text-muted)' }}>
                ...
                >
                  ...
                  <div className="p-2">
                    <p className="text-[10px] font-semibold" style={{ color: 'var(--preview-text)' }}>
                      Exemple de produit
                    </p>
                    <p className="text-[10px]" style={{ color: 'var(--preview-text-muted)' }}>
                      Description courte
                    </p>
                    <div className="mt-2 flex items-center justify-between">
                      <span className="text-xs font-bold" style={{ color: 'var(--preview-primary)' }}>
                        150,000 XOF
                      </span>
                      <span
                ...
                <div className="flex gap-2">
                  {(['success_color', 'warning_color', 'error_color', 'info_color'] as ColorKey[]).map((k) => (
                    <div
                      ...
                      style={{ backgroundColor: (colors[k] as string) || '#ccc' }}
                    />
                  ))}
                </div>
              </div>
