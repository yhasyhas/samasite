...
export function PromotionsPage() {
  ...
  const handleSave = async () => {
    ...
    if (editingPromo) {
      ...
      const after = { ...editingPromo, ...payload };
      await logActivityWithDiff(
        { action: 'update', entity_type: 'promotion', entity_id: editingPromo.id },
        editingPromo,
        after
      );
  ...
  const toggleActive = async (promo: PromotionWithLinks) => {
    ...
    await updatePromotion(promo.id, { is_active: newState });

    await logActivityWithDiff(
      { action: 'status_change', entity_type: 'promotion', entity_id: promo.id },
      { is_active: promo.is_active },
      { is_active: newState }
    );
                    ...
                    <tr key={promo.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs text-slate-500">
                        <span className="flex items-center gap-1">
                          <Calendar size={11} />
                          {promo.starts_at ? format(new Date(promo.starts_at), 'dd/MM/yy HH:mm', { locale: fr }) : 'Immédiate'}
                          {' → '}
                          {promo.ends_at ? format(new Date(promo.ends_at), 'dd/MM/yy HH:mm', { locale: fr }) : 'Sans fin'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })}
                placeholder="ÉTÉ2024" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 font-mono uppercase" />
            </div>
            <div className="col-span-2">
              <label className="text-sm font-medium text-slate-700 block mb-1">Libellé</label>
              <input type="text" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })}
                placeholder="Soldes d'été 2024" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2" />
            </div>
            <div>
          ...
          {form.applicable_to === 'specific_categories' && (
            <div>
              ...
              <select multiple value={form.linked_ids} onChange={(e) => setForm({ ...form, linked_ids: Array.from(e.target.selectedOptions).map(o => o.value) })}
                className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2" style={{ minHeight: '120px' }}>
