...
export function ProductsPage() {
  ...
  const handleSave = async () => {
    ...
    // ✅ Payload nettoyé : seulement les colonnes de la table products
    const payload = {
      name: form.name,
      sku: form.sku,
      slug,
      ...
      price: form.price,
      compare_at_price: form.compare_at_price ?? null, // ✅ null explicite pour effacer
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingProduct ? 'Modifier le produit' : 'Nouveau produit'} size="2xl">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="text-sm font-medium text-slate-700 block mb-1">Image URLs (une par ligne, format: url|alt)</label>
              <textarea
                rows={2}
                placeholder="https://example.com/image.jpg|Product image"
                value={form.images?.map((i) => `${i.url}|${i.alt}`).join('\n') || ''}
                onChange={(e) => {
                ...
                className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 resize-none font-mono"
              />
            </div>
...
//   const [form, setForm] = useState<Partial<Product>>({
//     name: '', sku: '', slug: '', description: '', short_description: '',
//     price: 0, compare_at_price: undefined, stock_qty: 0,
//     category_id: '', is_active: true, is_featured: false, images: [],
//   });
//   const [formErrors, setFormErrors] = useState<Record<string, string>>({});
//   const [saving, setSaving] = useState(false);
...
//   const openCreate = () => {
//     setEditingProduct(null);
//     setForm({ name: '', sku: '', slug: '', description: '', short_description: '', price: 0, compare_at_price: undefined, stock_qty: 0, category_id: '', is_active: true, is_featured: false, images: [] });
//     setFormErrors({});
//     setShowModal(true);
//   };
...
//   //   setSaving(true);
//   //   const slug = form.slug || slugify(form.name || '');
//   //   const payload = { ...form, slug, created_by: profile?.id };

//   //   if (editingProduct) {
//   //     const { error } = await updateProduct(editingProduct.id, payload);
//   //     if (error) { toast.error('Échec de la mise à jour du produit'); setSaving(false); return; }
//   //     await logActivity({ action: 'update', entity_type: 'product', entity_id: editingProduct.id, details: { name: form.name } });
//   //     toast.success('Produit mis à jour');
//   //   } else {
//   //     const { data, error } = await supabase.from('products').insert(payload).select().maybeSingle();
//   //     if (error) { toast.error('Échec de la création du produit: ' + error.message); setSaving(false); return; }
//   //     await logActivity({ action: 'create', entity_type: 'product', entity_id: data?.id, details: { name: form.name } });
//   //     toast.success('Produit créé');
//   //   }
...
//       {/* Create/Edit Modal */}
//       <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingProduct ? 'Edit Product' : 'New Product'} size="2xl">
//         <div className="space-y-4">
//           <div className="grid grid-cols-2 gap-4">
//             <div className="col-span-2">
//               <label className="text-sm font-medium text-slate-700 block mb-1">Nom *</label>
...
//             </div>
//             <div>
//               <label className="text-sm font-medium text-slate-700 block mb-1">Ancien prix</label>
//               <input
//                 type="number"
//                 min="0"
//                 value={form.compare_at_price || ''}
...
//             </div>
//             <div className="col-span-2">
//               <label className="text-sm font-medium text-slate-700 block mb-1">Image URLs (one per line, format: url|alt)</label>
//               <textarea
//                 rows={2}
//                 placeholder="https://example.com/image.jpg|Product image"
//                 value={form.images?.map((i) => `${i.url}|${i.alt}`).join('\n') || ''}
