...
import { Save, Globe, Phone, Mail, MapPin, DollarSign, Image, AlertTriangle, Palette } from 'lucide-react';
...
export function GeneralSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    const after = { ...settings, ...form };
    await logActivityWithDiff(
      { action: 'update', entity_type: 'setting', entity_id: settings.id },
      settings,
      after
    );
      ...
      <div className="space-y-5">
        <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <Field label="Nom de la boutique *">
            <input type="text" value={form.store_name || ''} onChange={(e) => setForm({ ...form, store_name: e.target.value })} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900" />
          </Field>
          <Field label="Slogan">
            <input type="text" value={form.store_tagline || ''} onChange={(e) => setForm({ ...form, store_tagline: e.target.value })} placeholder="Une phrase courte et percutante" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900" />
          </Field>
          <Field label="Description">
            <textarea rows={3} value={form.store_description || ''} onChange={(e) => setForm({ ...form, store_description: e.target.value })} placeholder="Décrivez votre boutique..." className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 resize-none" />
          </Field>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <Field label="Logo" icon={Image}>
            <SingleImageUploader value={form.logo_url || ''} onChange={(url) => setForm({ ...form, logo_url: url })} folder="logos" />
            {form.logo_url && (
          ...
          <Field label="Favicon">
            <SingleImageUploader value={form.favicon_url || ''} onChange={(url) => setForm({ ...form, favicon_url: url })} folder="logos" />
          </Field>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <Field label="Devise">
            <select value={form.currency || 'XOF'} onChange={(e) => setForm({ ...form, currency: e.target.value })} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <Field label="Téléphone" icon={Phone}>
            <input type="tel" value={form.contact_phone || ''} onChange={(e) => setForm({ ...form, contact_phone: e.target.value })} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900" />
          </Field>
          <Field label="Email" icon={Mail}>
            <input type="email" value={form.contact_email || ''} onChange={(e) => setForm({ ...form, contact_email: e.target.value })} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900" />
          </Field>
          <Field label="Adresse" icon={MapPin}>
            <textarea rows={2} value={form.contact_address || ''} onChange={(e) => setForm({ ...form, contact_address: e.target.value })} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 resize-none" />
          </Field>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <p className="text-sm text-slate-500">Choisissez l&apos;apparence de la bannière principale de votre page d&apos;accueil.</p>

          <div className="flex gap-2">
            <button
              onClick={() => setForm({ ...form, hero_mode: 'color' })}
              className={`flex-1 flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                form.hero_mode === 'color'
            ...
            <button
              onClick={() => setForm({ ...form, hero_mode: 'image' })}
              className={`flex-1 flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
          ...
          {form.hero_mode === 'image' && (
            <div className="space-y-4 pt-2">
              <Field label="Image de la bannière" icon={Image}>
                <SingleImageUploader
                ...
                />
                {form.hero_image_url && (
                  <div className="mt-3 w-full h-32 rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center border border-slate-200">
                    <img src={form.hero_image_url} alt="Hero preview" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                  </div>
                )}
              </Field>
              <Field label="Opacité de l&apos;assombrissement">
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                    min="0"
                    max="90"
                    value={form.hero_overlay_opacity ?? 50}
                    onChange={(e) => setForm({ ...form, hero_overlay_opacity: parseInt(e.target.value) })}
                    className="flex-1 accent-slate-900"
                  />
                  <span className="text-sm font-medium text-slate-700 w-12 text-right">{form.hero_overlay_opacity ?? 50}%</span>
                </div>
                <p className="text-xs text-slate-500 mt-1">Plus la valeur est élevée, plus le texte blanc sera lisible sur l&apos;image.</p>
              </Field>
            </div>
          )}
        </section>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-slate-900">Mode maintenance</h2>
              </div>
