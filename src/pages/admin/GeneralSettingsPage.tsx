...
export function GeneralSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    const { data, error } = await updateSettings(form);
    ...
    if (!data) {
      toast.error('Aucune donnée retournée par le serveur. Vérifiez les droits admin (RLS).');
      setSaving(false);
      return;
    }

    await logActivityWithDiff(
      { action: 'update', entity_type: 'setting', entity_id: data.id },
      settings,
      data
    );
      ...
      <div className="space-y-5">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Choisissez l&apos;apparence de la bannière principale de votre page d&apos;accueil.
          </p>
              ...
              <Field label="Opacité de l&apos;assombrissement">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-slate-700 w-12 text-right">
                    {form.hero_overlay_opacity ?? 50}%
                  </span>
                </div>
                <p className="text-xs text-slate-500 mt-1">
                  Plus la valeur est élevée, plus le texte blanc sera lisible sur l&apos;image.
                </p>
              </Field>
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                Lorsque cette option est activée, le site public affichera une page de maintenance.
              </p>
            </div>
            <button
              onClick={() => setForm({ ...form, maintenance_mode: !form.maintenance_mode })}
              className={`relative w-12 h-6 rounded-full transition-colors ${
                form.maintenance_mode ? 'bg-amber-500' : 'bg-slate-200'
              }`}
            >
              <div
                className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
                  form.maintenance_mode ? 'translate-x-6' : 'translate-x-0.5'
                }`}
              />
            </button>
          </div>
