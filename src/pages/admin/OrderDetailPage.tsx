    ...
    <div className="p-6 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">{format(new Date(order.created_at), 'd MMMM yyyy HH:mm', { locale: fr })}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4">
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex justify-between"><span className="text-slate-500">Source</span><span className="capitalize font-medium">{SOURCE_LABELS[order.source] || order.source}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Statut</span><StatusBadge status={order.status} type="order" /></div>
            <div className="flex justify-between"><span className="text-slate-500">Créée</span><span>{format(new Date(order.created_at), 'd MMM yyyy', { locale: fr })}</span></div>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100">
              ...
              <th className="text-right px-5 py-2.5 text-xs font-semibold text-slate-500 uppercase">Qté</th>
              ...
              <tr key={item.id}>
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-slate-900">{item.product?.name || 'Produit supprimé'}</span>
                  </div>
                </td>
